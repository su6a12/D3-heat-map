$(document).ready(function() {
	...
	$.getJSON("https://raw.githubusercontent.com/FreeCodeCamp/ProjectReferenceData/master/global-temperature.json", function(data) {
      ...
      return baseTemp + each.variance;
		...
		// set the row ticks to equal about the number of decades in the data
		var row_num = Math.floor((tempsData.length / 12) / 10);	
		...
		var RectLength = 38;
		// set legend size relative to rectangle size
		...
		var yAxis = heatMap.append("g")
						...
						.attr("x", 0)
						.attr("y", function(d, i) { return (i+1) * RectLength; })
		...
		legend.append("text")
						...
						.attr("x", function(d, index) { return (legendWidth * index - 10) + (innerWidth - legendWidth * 8); })
		...
		rectangles.on("mouseover", function(each) {
						tooltip.transition()
						.style("opacity", 0.7);
